...
class MappingWorkspace {
  ...
  loadPayloadFromFile(filePath) {
    ...
    logger.info('Loaded incoming payload', { filePath: path.resolve(filePath) });
    return this.payload;
  }

  loadSuiteQlFromFile(filePath) {
    this.suiteQlResult = readJson(filePath);
    logger.info('Loaded SuiteQL response', { filePath: path.resolve(filePath) });
  ...
  runMappings(context = {}) {
    if (!this.payload || !this.suiteQlResult) {
      throw new Error('Load payload and SuiteQL response before running mappings');
    }
    const results = {};
  ...
  summarizeDifferences() {
    ...
    const onlyInPayload = [...payloadKeys].filter((key) => !suiteQlKeys.has(key));
    const onlyInSuiteQl = [...suiteQlKeys].filter((key) => !payloadKeys.has(key));
    ...
    logger.info('Generated structural comparison between payload and SuiteQL response', {
